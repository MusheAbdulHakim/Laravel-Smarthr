<div class="modal-body">
    <form action="{{ route('designations.update', $designation->id) }}" method="post" enctype="multipart/form-data">
        ...
        <x-form.input-block>
            ...
            <x-form.input type="text" name="name" placeholder="{{ __('Enter Designation') }}" value="{{ $designation->name }}" />
        </x-form.input-block>
        <x-form.input-block>
            <x-form.label>{{ __('Description') }}</x-form.label>
            <x-form.textarea name="description">{{ $designation->description }}</x-form.textarea>
        </x-form.input-block>
