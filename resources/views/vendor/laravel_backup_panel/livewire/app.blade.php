<div class="container mb-5">
    <div class="d-flex align-items-end pt-4">
        ...
        <button id="create-backup" class="btn btn-primary btn-sm ml-auto px-3" @click="$wire.funBackup()">
            Create Backup
        </button>
        <div class="dropdown ml-3">
            ...
            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                <a class="dropdown-item" href="#" id="create-backup-only-db" @click="$wire.funBackup('only-db')">
                    Create database backup
                </a>
                <a class="dropdown-item" href="#" id="create-backup-only-files" @click="$wire.funBackup('only-files')">
                    Create files backup
                </a>
            </div>
        </div>
    ...
    <div x-data="{
        init() {
            document.addEventListener('livewire:initialized', () => {
                $wire.updateBackupStatuses()
                $wire.on('backupStatusesUpdated', function() {
                    $wire.getFiles()
                })
                $wire.on('showErrorToast', function(message) {
                    Toastify({
                        text: message,
                        duration: 10000,
                        gravity: 'bottom',
                ...
                $wire.on('showDeleteModal', function() {
                    deleteModal.modal('show')
                })
                $wire.on('hideDeleteModal', function() {
                    deleteModal.modal('hide')
                })
    
                deleteModal.on('hidden.bs.modal', function() {
                    $wire.deletingFile = null
                })
            })
        }
    }">
